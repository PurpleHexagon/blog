  ...
  return (
    <div className={className}>
      ...
      {siteConfig.jobs && siteConfig.jobs.map(job => {
        ...
        if (job.keyAccomplishments) {
          job.keyAccomplishments.forEach((keyAccomplishments) => {
            kA.push(<li>{keyAccomplishments}</li>)
          });
        }


        return <article key={job.begin.month + job.begin.year} className='timeline__item'>
          <div className="inner">
            <span className="timeline__date">
              <span className="timeline__month">{job.begin.month}</span>
              <span className="timeline__year">{job.begin.year}</span>
            </span>
            <h2 className='timeline__title'>{job.company}</h2>
            <div className='timeline__title timeline__title--shrink'>{job.occupation}
            < br/><small
